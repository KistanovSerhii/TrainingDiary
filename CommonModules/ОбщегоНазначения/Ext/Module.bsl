...
Функция ПолучитьВесСпортсменаСрезПоследних(Период, Спортсмен) Экспорт
	...
	Запрос.Текст = 
		...
		|	АнтропометрияСрезПоследних.ПВХ 			КАК Вес,
		|	АнтропометрияСрезПоследних.Значение 	КАК Значение
		|ИЗ
		|	РегистрСведений.Антропометрия.СрезПоследних(&Период,
		|	Спортсмен = &Спортсмен И Значение > 0
		|	И ПВХ.ИмяПредопределенныхДанных = &ВесПВХ) КАК АнтропометрияСрезПоследних";
	
	Запрос.УстановитьПараметр("Период", 	Период);
	Запрос.УстановитьПараметр("Спортсмен", 	Спортсмен);
	Запрос.УстановитьПараметр("ВесПВХ", 	Строка(ПредопределенноеЗначение("ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения.Вес").Наименование));
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		результатВес = ВыборкаДетальныеЗаписи.Значение;
	КонецЕсли;	
